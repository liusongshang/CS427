...
public class GetVisitRemindersActionTest extends TestCase {
	...
	public void testNormalReturn() throws Exception {
		...
		action.getVisitReminders(GetVisitRemindersAction.ReminderType.IMMUNIZATION_NEEDERS);
	}
    ...
    public void testImmunization() throws Exception {
        gen.standardData();
        action = new GetVisitRemindersAction(factory, 2L);

        List<VisitReminderReturnForm> imm = action.getVisitReminders(GetVisitRemindersAction.ReminderType.IMMUNIZATION_NEEDERS);
        assertEquals(imm.size(), 0);
    }    
