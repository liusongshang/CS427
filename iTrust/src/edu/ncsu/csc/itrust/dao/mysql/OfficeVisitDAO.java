...
public class OfficeVisitDAO {
	...
	/**
	 * Returns a list of all office visits in the DAOFactory
	 *
	 * This is added for UC20, see:
	 *
	 * ViewCODTrendsReportAction.java
	 *
	...
	public List<OfficeVisitBean> getAllOfficeVisits() throws DBException{
		...
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT * FROM officevisits ORDER BY ID DESC");
			ResultSet rs = ps.executeQuery();
			List<OfficeVisitBean> loadlist = officeVisitLoader.loadList(rs);
			rs.close();
			ps.close();
