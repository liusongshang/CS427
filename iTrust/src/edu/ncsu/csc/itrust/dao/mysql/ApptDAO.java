...
public class ApptDAO {
	...
	public List<ApptBean> getApptDays(String lowdata, String highdata)
			throws FormValidationException, ITrustException, DBException {
		java.util.Date dateL= new Date();
		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		java.util.Date dateH= new Date();
		Connection conn = null;
		PreparedStatement pstring = null;
		
		try{
			dateL = dateFormat.parse(lowdata);
			dateH = dateFormat.parse(highdata);
			if (dateL.after(dateH))
				throw new FormValidationException("The Later Data should be bigger.");
		}catch (Exception e) {
			throw new FormValidationException("Format of Data Wrong");
		}
		...
		try {
			conn = factory.getConnection();
			pstring = conn.prepareStatement(
					"SELECT * FROM appointment WHERE sched_date >= ? AND sched_date <= ? ORDER BY patient_id DESC");
			pstring.setTimestamp(1, new Timestamp(dateL.getTime()));
			pstring.setTimestamp(2, new Timestamp(dateH.getTime()));
