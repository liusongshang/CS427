...
public class AuthDAO {
	 ...
	 * UC30 S7 Filter
	 *
	 * Change the messageFilter of a particular user
	 *
	 * @param mid The MID of the user whose password we are changing.
	 * @param messageFilter The new messageFilter for user
	 * @throws DBException
	 */
	public void setMessageFilter(final long mid, final String messageFilter) throws DBException {
		Connection conn = null;
		PreparedStatement pstmt = null;
		try {
			conn = factory.getConnection();
			pstmt = conn.prepareStatement("UPDATE users SET messageFilter=? WHERE MID=?");
			pstmt.setString(1, messageFilter);
			pstmt.setLong(2, mid);
			pstmt.executeUpdate();
			pstmt.close();
		} catch (SQLException e) {
	 ...
	 * UC30 S7 Filter
	 *
	 * Return the messageFilter for a particular user.
	 *
	 * @param mid The MID of the user we are looking up.
	 * @return The messageFilter of the user we are looking up.
	 * @throws ITrustException
	 */
	public String getMessageFilter(final long mid) throws ITrustException {
		Connection conn = null;
		PreparedStatement pstmt = null;
		try {
			conn = factory.getConnection();
			pstmt = conn.prepareStatement("SELECT messageFilter FROM users WHERE MID=?");
			pstmt.setLong(1, mid);
			final ResultSet results = pstmt.executeQuery();
			String result = "";
			if (results.next()){
				result = results.getString("messageFilter");
			}
			results.close();
			pstmt.close();
			return result;
