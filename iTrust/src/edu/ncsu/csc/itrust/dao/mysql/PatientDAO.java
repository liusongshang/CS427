...
/**
     ...
     * @throws DBException
     */
    public List<PatientBean> getAll_PreRegistered_Patients() throws DBException {
    Connection conn = null;
    PreparedStatement ps = null;
    try {
        conn = factory.getConnection();
        ps = conn.prepareStatement("SELECT patients.* FROM patients,users where users.role ='preregistered' and patients.mid=users.mid");
        ResultSet rs = ps.executeQuery();
        List<PatientBean> loadlist = patientLoader.loadList(rs);
        rs.close();
        ps.close();
    ...
    /**
     * Return a list of all email addresses of patients in the database.
     */
    public ArrayList<String> getAllEmails() throws DBException {
        ArrayList<String> emailSet = new ArrayList<String>();
        Connection conn = null;
        PreparedStatement ps = null;
        try {
            conn = factory.getConnection();
            ps = conn.prepareStatement("SELECT email FROM patients;");
            ResultSet results = ps.executeQuery();
            while (results.next()) {
                emailSet.add(results.getString(1));
            }
            ps.close();
            return emailSet;
        } catch (SQLException e) {
    ...
    /**
     * Deactivate a preregistered patient and hide it from the
     *
     * @param mid The MID of the user as a long.
     * @throws DBException
     */
    public void deactivatePrePatient(final long mid) throws ITrustException,DBException {
        Connection conn = null;
        ...
        try {
            ...
            ps2 = conn.prepareStatement("SELECT * FROM users WHERE MID = ?");
            ps2.setLong(1, mid);
            ResultSet rs;
            rs = ps2.executeQuery();
            if (rs.next()) {
            ...
            pstmt = conn.prepareStatement("UPDATE users SET role = ? WHERE MID = ?");
 ...
 /**
     * Activate a preregistered patient into a regular patient
     *
     * @param mid The MID of the user as a long.
     * @throws DBException
     */
    public void ActivatePrePatient(final long mid) throws ITrustException,DBException {
        Connection conn = null;
        PreparedStatement pstmt = null;
        PreparedStatement ps = null;
        try {
