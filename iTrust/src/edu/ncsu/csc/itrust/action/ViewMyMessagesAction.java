...
public class ViewMyMessagesAction {
	...
	/**
	 * Gets all the types of information emails can be sorted by
	 * 
	 * @return List of Strings correlating to all sortBy types
	 */
	public List<String> getSortTypes() {
		List<String> types = new ArrayList<String>();
		types.add("Recipient/Sender");
		types.add("Timestamp");
		return types;
	}
	
	/**
	 * Gets all the directions emails can be sorted in (asc/desc)
	 * 
	 * @return List of Strings correlating to all sort directions
	 */
	...
	public List<MessageBean> getAllMyMessagesTimeAscending() throws SQLException, DBException {
		// Ascending should be A to Z. Descending seems to do this.
	...
	/**
	 * Gets all the messages for the logged in user and sorts time in descending order
	 * 
	 * @return a list of all the user's messages
	 * @throws SQLException
	 * @throws DBException 
	...
	public List<MessageBean> getAllMyMessagesTimeDescending() throws SQLException, DBException {
		// Descending should be Z to A. Ascending seems to do this.
	 ...
	 *
	 * Validate filter
	 * If validation fails, filter should start with "Error"
	 * Two cases for failing: hasWord.contains(notWords) or vice versa; startDate > endDate
	 * @param filter
	 * @return
	 */
	public String validateAndCreateFilter(String filter) throws ParseException {

		boolean error = false;
		String[] f = filter.split(",", -1);
		String reason = "";

		String messageBodyMustContain = f[2];
		String messageBodyMustNotContain = f[3];
		String startDate = f[4];
		String endDate = f[5];

		// check if hasWords contains noWords, or vice versa
		if(!messageBodyMustContain.equals("") && !f[3].equals("")) {
			Boolean containIncludesNotContain = messageBodyMustContain.toLowerCase().contains(messageBodyMustNotContain.toLowerCase());
			Boolean notContainIncludesContain = messageBodyMustNotContain.toLowerCase().contains(messageBodyMustContain.toLowerCase());
			if(containIncludesNotContain || notContainIncludesContain){
				error = true;
				reason = "message cannot contain '" + messageBodyMustContain + "' and not contain '"+ messageBodyMustNotContain + "' at the same time";
			}
		}
		...
		if(!startDate.equals("") && !endDate.equals("")){
			DateFormat format = new SimpleDateFormat("MM/dd/yyyy");
			Date sStart = format.parse(startDate);
			Date sEnd = format.parse(endDate);
			if (sStart.after(sEnd)){
				error = true;
				reason = "start date " + startDate + " cannot be after end date " + endDate;
			}
		}
