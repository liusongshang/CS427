...
public class ViewHealthRecordsHistoryAction {
    ...
    return calculateAgeInMonthsAtDate(ov.getVisitDate());
	}	

	/**
	 * Calculates the patient's age at a date. Reads in an office visit ID and gets 
	 * its visit date to check against the patient's birthday. Returns the patient's age in months.
	 * @param atDate Date to calculate at
	 * @return the patient's age in months
	 * @throws DBException
	 */
	public int calculateAgeInMonthsAtDate(Date atDate) throws DBException {
		//Create int for patient's age in months
		int ageInMonths = 0;
		...
		int visitDay = date.get(Calendar.DAY_OF_MONTH);
		int visitMonth = date.get(Calendar.MONTH) + 1;
		int visitYear = date.get(Calendar.YEAR);
