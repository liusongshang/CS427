	...
	public DiagnosisStatisticsBean getDiagnosisStatistics(String lowerDate, String upperDate, String icdCode, String zip) throws FormValidationException, ITrustException {
		...
		try {
			...
			if (lower.after(upper))
				throw new FormValidationException("Start date must be before end date. Please try again.");
			
			if (!zip.matches("([0-9]{5})|([0-9]{5}-[0-9]{4})"))
				throw new FormValidationException("Zip Code must be 5 digits. Please try again.");
			...
			if (validCode == false) {
				throw new FormValidationException("ICDCode must be valid diagnosis code. Please try again.");
			}

			dsBean = diagnosesDAO.getDiagnosisCounts(icdCode, zip, lower, upper);
			
		} catch (ParseException e) {
			throw new FormValidationException("Invalid date format entered. Please enter dates in MM/dd/yyyy and try again.");
		} 
	...
	public ArrayList<DiagnosisStatisticsBean> getEpidemicStatistics(String startDate, String icdCode, String zip, String threshold) throws FormValidationException, DBException {
		...
		if (dbList.size() > 0) {
			for (DiagnosisStatisticsBean d : dbList) {
				...
				avgState += d.getStateStats();
				avgOverall += d.getOverallStats();
			}
			avg /= dbList.size();
			avgRegion /= dbList.size();
			avgState /= dbList.size();
			avgOverall /= dbList.size();
		}
		
		dbAvg.setOverallStats(avgOverall);
		dbAvg.setStateStats(avgState);
