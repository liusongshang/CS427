...
<div align=center>
  ...
  int patAgeOV = 0;
    ...
    if ((hr.getOfficeVisitID()==0L)) {
        Date visitDate = hr.getVisitDate();
		    patAgeOV = historyAction.calculateAgeInMonthsAtDate(visitDate);
    }
    else {
		    patAgeOV = historyAction.calculateAgeInMonthsAtOfficeVisit(hr.getOfficeVisitID());
    }
		...
		if(patAgeOV < 36){
			...
			if(babyTripper == false && !records.isEmpty()){
				%>
				<tr>
				<th colspan="10" style="text-align: center;"><%= patientName %>'s Basic Infant Health History</th>
				</tr>
				<tr class = "subHeader">
					<td>Date</td>
          <td>Visit or Edit</td>
					...
					<td>Head Circumference<br />(<a href="/iTrust/auth/hcp-uap/healthDataChart.jsp?dataType=HeadCirc" id="viewHeadCircChart">View Chart</a>)</td>
			...
			if(babyTripper == true && !records.isEmpty()){
				...
				<tr>
					...
					<td align=center><%= StringEscapeUtils.escapeHtml((hr.getOfficeVisitID()==0L)?"Edit":"Visit")%></td>
					...
					<td align=center><%= StringEscapeUtils.escapeHtml("" + (hr.getEditReason())) %></td>
				</tr>
				<%
			}
		...
		//If the patient is less than 144 months (12 years) old
			...
			if(youthTripper == false && !records.isEmpty()){
				...
				last recorded date, and recorded by personnel fields.
				 -->
				<tr>
					<th colspan="10" style="text-align: center;"><%= patientName %>'s Basic Youth Health History</th>
				</tr>
				<tr class = "subHeader">
					<td>Date</td>
          <td>Visit or Edit</td>
		...
		//If patient is 12 years or older
			...
			if(adultTripper == false && !records.isEmpty()){
				...
				cholesterol, last recorded date, and recorded by personnel fields.
				 -->
				<tr>
					<th colspan="16" style="text-align: center;"><%= patientName %>'s Basic Adult Health History</th>
				</tr>
				<tr class = "subHeader">
					<td>Date</td>
          <td>Visit or Edit</td>
