...
<%
	...
	if(request.getParameter("edit") != null && request.getParameter("edit").equals("true")) {
		...
		if(request.getParameter("cancel") != null) 
			...
			//Validate Filter
			nf = action.validateAndCreateFilter(nf); // TODO validate filter
			...
			if(!error) {
				if(request.getParameter("test") != null) {
					...
					f_action.editMessageFilter(nf); // TODO store / update filter in patientBean
		...
		if(request.getParameter("testFilter") != null) {
			...
			String filter = dao.getPatient(loggedInMID.longValue()).getMessageFilter(); // TODO retrieve filter from patientBean
			if(!filter.equals("")) {
				String[] f = filter.split(",", -1);
				for(i=0; i<6; i++) {
					try {
	...
	if((request.getParameter("filter") != null && request.getParameter("filter").equals("true")) || request.getParameter("testFilter") != null) {
		...
		if(request.getParameter("testFilter") != null) {
			...
			filter = dao.getPatient(loggedInMID.longValue()).getMessageFilter(); // TODO
		}
